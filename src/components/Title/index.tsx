...
export const Title = ({children, tag='h1', isBigText=false, className=''}:TitleProps) => {
  ...
  if (tag === 'h1') return <h1 className={`title ${isBigText? 'text-6xl':'text-3xl'} ${className} `}>{children}</h1>
  if (tag === 'h2') return <h2 className={`title text-xl ${className} `}>{children}</h2>
